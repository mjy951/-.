...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  const left = (width - Math.max(...lines.map(line => measureLine(ctx, line, letterSpacing)))) / 2;
  lines.forEach((line, index) => drawLine(ctx, line, left + measureLine(ctx, line, letterSpacing) / 2, startY + index * lineHeight, letterSpacing));
